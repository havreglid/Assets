...
// BUG - Female Great People - start
		...
		// && (!GC.getUnitInfo((UnitTypes)iI).isFemale())) required here to prevent duplicate "Can train" messages in 'Pedia
		if ((GC.getUnitInfo((UnitTypes)iI).getPrereqBuilding() == eBuilding) && (!GC.getUnitInfo((UnitTypes)iI).isFemale()))  // && (!GC.getUnitInfo((UnitTypes)iI).isFemale())) required here
// BUG - Female Great People - end		
		{
