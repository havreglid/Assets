				...
				if (!GET_PLAYER(eCapturingPlayer).isHuman())
				{
					...
					if (pPlot && !pPlot->isCity(false))
					{
						if (GET_PLAYER(eCapturingPlayer).AI_getPlotDanger(pPlot) && GC.getDefineINT("AI_CAN_DISBAND_UNITS"))
						{
							...
							if ( (GC.getDefineINT("BBAI_UNIT_CAPTURE") >= 2) ||	( ((GC.getDefineINT("BBAI_UNIT_CAPTURE") + ((GET_PLAYER(eOwner).isHuman())? 0 : 1)) == 2)
								&& (pPlot->getOwner() == eCapturingPlayer || pPlot->isAdjacentPlayer(eCapturingPlayer, (pkCapturedUnit->getDomainType() == DOMAIN_LAND && 
								!pkCapturedUnit->canMoveAllTerrain()))) ) )
							{
