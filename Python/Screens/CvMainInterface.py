...
## Specialist Count ##
		self.iMaxPerColumn	 = 4	## Define max number of specialists per column
## Specialist Count ##
...
# BAT - HUD Civics - begin		
		if MainOpt.isHUDCivics():	
			global g_bTurnLogUp
			activePlayer = gc.getActivePlayer()
			Fy = 100
			if (CyInterface().isCityScreenUp() == False): # and (g_bTurnLogUp == False):
				
				# Show the civics if we're in the main screen
				screen.setText( "CivicTitle", "Background", "<color=0,255,0,0>Player Civics</color>", CvUtil.FONT_LEFT_JUSTIFY, xResolution - 170, Fy, -0.3, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
				screen.moveToBack( "CivicTitle" )
				Fy += 23
				
				for j in range (gc.getNumCivicOptionInfos()):	
					szName = "CivicsText" + str(j)
					screen.addCheckBoxGFC("CivicsScreenButton" + str(j), gc.getCivicInfo(activePlayer.getCivics(j)).getButton(), ArtFileMgr.getInterfaceArtInfo("BUTTON_HILITE_SQUARE").getPath(), xResolution - 170, Fy, 20, 20, WidgetTypes.WIDGET_GENERAL, -1, -1, ButtonStyles.BUTTON_STYLE_LABEL)
					screen.setText( szName, "Background", gc.getCivicInfo(activePlayer.getCivics(j)).getDescription(), CvUtil.FONT_LEFT_JUSTIFY, xResolution - 144, Fy, -0.3, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
					screen.moveToBack( szName )
					screen.moveToBack( "CivicsScreenButton" )
					
					Fy += 23
				
			# If the city screen is up don't show the civics	
			else:
				screen.hide("CivicTitle")
				for j in range (gc.getNumCivicOptionInfos()):	
					screen.hide("CivicsScreenButton" + str(j))
					szName = "CivicsText" + str(j)
					screen.hide( szName )
				#g_bTurnLogUp = False
...
## Specialist Count ##
		...
		screen.hide("CityAngryCitizenCount")
		screen.hide("IncreaseMaxRow")
		screen.hide("DecreaseMaxRow")
			
		for i in xrange(gc.getNumSpecialistInfos()):
			screen.hide("IncreaseSpecialist" + str(i))
			screen.hide("DecreaseSpecialist" + str(i))
			screen.hide("CitizenButton" + str(i))
			screen.hide("CitySpecialistCount" + str(i))

		pHeadSelectedCity = CyInterface().getHeadSelectedCity()

		if ( CyInterface().isCityScreenUp() ):
			if (pHeadSelectedCity and CyInterface().getShowInterface() == InterfaceVisibility.INTERFACE_SHOW):
				iAngryCount = pHeadSelectedCity.angryPopulation(0)
				screen.show("AngryCitizen")
				szBuffer = u"<font=2><color=255,255,120,0>"  + u"%d" %(iAngryCount) + u"</color></font>"
				screen.setText("CityAngryCitizenCount", "Background", szBuffer, CvUtil.FONT_CENTER_JUSTIFY, xResolution - 74, yResolution - 238, 0, FontTypes.GAME_FONT, WidgetTypes.WIDGET_GENERAL, -1, -1 )
				screen.setStyle("CityAngryCitizenCount", "Button_Stone_Style" )
				screen.show("CityAngryCitizenCount")
				screen.show("IncreaseMaxRow")
				if self.iMaxPerColumn > 1:
					screen.show("DecreaseMaxRow")
...
# BUG - Progress Bar - Tick Marks - end
				...
				iCount = 0
## Platy Display ##
				screen.addTableControlGFC( "TradeRouteTable", 2, 10, 187, 238, 98, False, False, 24, 24, TableStyles.TABLE_STYLE_STANDARD )
				screen.setStyle( "TradeRouteTable", "Table_City_Style" )
				screen.addTableControlGFC( "BuildingListTable", 2, 10, 317, 238, yResolution - 541, False, False, 24, 24, TableStyles.TABLE_STYLE_STANDARD )
				screen.setStyle( "BuildingListTable", "Table_City_Style" )
				screen.setTableColumnHeader( "TradeRouteTable", 0, u"", 110 )
				screen.setTableColumnHeader( "TradeRouteTable", 1, u"", 116 )
				screen.setTableColumnRightJustify( "TradeRouteTable", 1 )

				screen.setTableColumnHeader( "BuildingListTable", 0, u"", 110 )
				screen.setTableColumnHeader( "BuildingListTable", 1, u"", 116 )
...
## Platy Nationality Display ##
				screen.addTableControlGFC( "NationalityText", 1, 16 - 4, yResolution - 213, 220, iStackBarHeight, False, False, 24, 24, TableStyles.TABLE_STYLE_EMPTY )
				screen.setTableColumnHeader( "NationalityText", 0, u"", 220 )
...
## Platy Great People Display ##
				if ((pHeadSelectedCity.getGreatPeopleProgress() > 0) or (pHeadSelectedCity.getGreatPeopleRate() > 0)):
					...
					lGreatPeople = ["UNITCLASS_PROPHET", "UNITCLASS_ARTIST", "UNITCLASS_SCIENTIST", "UNITCLASS_MERCHANT", "UNITCLASS_ENGINEER", "UNITCLASS_GREAT_SPY"]
					lGreatUnit = [(0,0)] * len(lGreatPeople)
					iTotalProgress = 0
					iFirst = -1
					for i in xrange(len(lGreatPeople)):
						iUnitClass = gc.getInfoTypeForString(lGreatPeople[i])
						iUnit = gc.getCivilizationInfo(gc.getPlayer(pHeadSelectedCity.getOwner()).getCivilizationType()).getCivilizationUnits(iUnitClass)
						iProgress = pHeadSelectedCity.getGreatPeopleUnitProgress(iUnit)
						lGreatUnit[i] = (iProgress, iUnit)
						if iProgress > 0:
							if iFirst == -1:
								iFirst = i
							iTotalProgress += iProgress

					iPercentLeft = 100
					for i in xrange(len(lGreatUnit)):
						iProgress = lGreatUnit[i][0]
						if iProgress > 0:
							iPercent = iProgress * 100 / iTotalProgress
							iPercentLeft -= iPercent
							lGreatUnit[i] = (iPercent, lGreatUnit[i][1])
					if iFirst > -1:
						lGreatUnit[iFirst] = (lGreatUnit[iFirst][0] + iPercentLeft, lGreatUnit[iFirst][1])
					lGreatUnit.sort()
					iProgress = lGreatUnit[len(lGreatUnit) -1][0]
					szText = ""
					sButton = ""
					if iProgress > 0:
						szText = str(iProgress) + "%"
						iUnit = lGreatUnit[len(lGreatUnit) -1][1]
						sButton = gc.getUnitInfo(iUnit).getButton()

					screen.addTableControlGFC( "GreatPeopleText", 2, xResolution - 246 - 4, yResolution - 187, 236, iStackBarHeight, False, False, 24, 24, TableStyles.TABLE_STYLE_EMPTY )
					screen.setTableColumnHeader( "GreatPeopleText", 0, u"", 75 )
					screen.setTableColumnHeader( "GreatPeopleText", 1, u"", 236 - 75 )
					screen.appendTableRow( "GreatPeopleText" )
					screen.setTableText( "GreatPeopleText", 0, 0, szText, sButton, WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
					screen.setTableText( "GreatPeopleText", 1, 0, szBuffer, "", WidgetTypes.WIDGET_GENERAL, -1, -1, CvUtil.FONT_LEFT_JUSTIFY)
...
## Platy City Display ##
		screen.addTableControlGFC( "SelectedUnitText", 2, 10, yResolution - 109, 183, 102, False, False, 24, 24, TableStyles.TABLE_STYLE_STANDARD )
		screen.setStyle( "SelectedUnitText", "Table_EmptyScroll_Style" )
		screen.hide( "SelectedUnitText" )
		screen.hide( "SelectedUnitLabel" )
		
		screen.addTableControlGFC( "SelectedCityText", 3, 10, yResolution - 139, 200, 128, False, False, 24, 24, TableStyles.TABLE_STYLE_STANDARD )
...
## Specialist Count ##
		if inputClass.getNotifyCode() == NotifyCode.NOTIFY_CLICKED:
			if inputClass.getFunctionName() == "IncreaseMaxRow":
				self.iMaxPerColumn += 1
				self.setSpecialistIcons()
				self.updateCitizenButtons()
			elif inputClass.getFunctionName() == "DecreaseMaxRow":
				self.iMaxPerColumn -= 1
				self.setSpecialistIcons()
				self.updateCitizenButtons()
#			return 0
...
# BAT - HUD Civics - Start
		# Hides the civics list
		global g_bTurnLogUp
		if inputClass.getNotifyCode() == NotifyCode.NOTIFY_CLICKED:
			if inputClass.getFunctionName() == "TurnLogButton":
				screen = CyGInterfaceScreen( "MainInterface", CvScreenEnums.MAIN_INTERFACE )
				screen.hide("CivicTitle")
				for j in range (gc.getNumCivicOptionInfos()):	
					screen.hide("CivicsScreenButton" + str(j))
...
# BUG - Raw Yields - end
	...
	def setSpecialistIcons(self):
		# Angry Citizens
		screen = CyGInterfaceScreen( "MainInterface", CvScreenEnums.MAIN_INTERFACE )
		xResolution = screen.getXResolution()
		yResolution = screen.getYResolution()
		screen.setImageButton("AngryCitizen", ArtFileMgr.getInterfaceArtInfo("INTERFACE_ANGRYCITIZEN_TEXTURE").getPath(), xResolution - 74, yResolution - 238, 24, 24, WidgetTypes.WIDGET_ANGRY_CITIZEN, -1, -1 )
		screen.hide("AngryCitizen")	

		szBuffer = u"<font=4>" + u"%c" %(CyGame().getSymbolID(FontSymbols.GOLDEN_AGE_CHAR)) + u"</font>"
		screen.setText("IncreaseMaxRow", "Background", szBuffer, CvUtil.FONT_LEFT_JUSTIFY, xResolution - 46, yResolution - 236, -0.3, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, 7777, -1 )
		screen.hide("IncreaseMaxRow")
		szBuffer = u"<font=4>" + u"%c" %(CyGame().getSymbolID(FontSymbols.BAD_GOLD_CHAR)) + u"</font>"
		screen.setText("DecreaseMaxRow", "Background", szBuffer, CvUtil.FONT_LEFT_JUSTIFY, xResolution - 24, yResolution - 236, -0.3, FontTypes.SMALL_FONT, WidgetTypes.WIDGET_GENERAL, 7878, -1 )
		screen.hide("DecreaseMaxRow")
		iCount = 1
		for i in xrange( gc.getNumSpecialistInfos() ):
			if (gc.getSpecialistInfo(i).isVisible()):
