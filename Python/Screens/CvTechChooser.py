...
## CvTechChooser.py
## Merged Platyping's Advanced Tech Screen with BUG Tech Screen - 07/07/13 - Lemon Merchant
...
## Hide Era Start ##
		self.iHideEra = CyGame().getStartEra() + 1
## Hide Era End ##
...
## Hide Era ##
		szDropdownName = str("PlatyHideEra")
		screen.addDropDownBoxGFC(szDropdownName, screen.getXResolution() - 300, 8, 180, WidgetTypes.WIDGET_GENERAL, -1, -1, FontTypes.GAME_FONT)
		for iEra in xrange(gc.getNumEraInfos()):
			screen.addPullDownString(szDropdownName, CyTranslator().getText("TXT_KEY_SHOW_ERA", (gc.getEraInfo(iEra).getDescription(),)), iEra, iEra, iEra == self.iHideEra)
## Hide Era ##
...
#		BugUtil.debug("cvTechChooser: placeTechs")
		...
		for i in xrange(gc.getNumTechInfos()):
## Hide Era Start ##
			if gc.getTechInfo(i).getEra() > self.iHideEra:
				continue
	## Hides Disabled Techs ##
			if not pPlayer.canEverResearch(i):
				continue
	## Hides Disabled Techs ##
...
## Hide Era Start ##
			if gc.getTechInfo(i).getEra() > self.iHideEra: continue
## Hide Era End ##		
	## Hides Disabled Techs ##
			if not pPlayer.canEverResearch(i):
				continue
	## Hides Disabled Techs ##
		...
		for i in range(gc.getNumTechInfos()):
# Start - Keeps tech screen from printing text from unshown techs			
			if gc.getTechInfo(i).getEra() > self.iHideEra: continue
# End - LM 07/07/13
...
# Hide the arrows for invisible techs			
			if gc.getTechInfo(i).getEra() > self.iHideEra:
				continue
# End hide the arrows for invisible techs
			if bANDPreReq:
...
## Hide Era Start ##
			if (inputClass.getFunctionName() == "PlatyHideEra"):
				self.handlePlatyHideEraCB(inputClass.getData())
				return 1
## Hide Era End ##
		...
		return 0
## Hide Era Start ##
	def handlePlatyHideEraCB ( self, argsList ) :
		self.iHideEra = int(argsList)
